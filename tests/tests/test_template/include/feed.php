...
function get_blog_post($numberOfBlogPost)
{
    ...
    $link = str_replace('?utm_source=rss&amp;utm_medium=rss&amp;utm_content=rss?utm_source=rss&utm_medium=rss&utm_content=rss', '?utm_source=qa_report&utm_medium=qa_report&utm_content=qa_report', $link);
    $descriptionShort = get_string_between($description, '<div class="clearfix text-formatted field field--name-field-d-long-text field--type-text-long field--label-hidden field__item"><p>', ' </div> </div>');
    $descriptionShort = strip_tags($descriptionShort, "<a>");
    return $blog_post = array("title" => $title, "image" => $image, "link" => $link, "description" => $descriptionShort, "pubDate" => $pubDate);
}

function display_blog_post($numberOfBlogPost)
{
    $blog_post = get_blog_post($numberOfBlogPost);
    $blog_post_description = ' - ' . implode(' ', array_slice(explode(' ', $blog_post["description"]), 0, 80)) . "...";
